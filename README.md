...
The controlled model scaling experiments use three instruction tuned models from the Qwen2.5 family. Falcon3 3B Instruct is included as a second model family for cross-family validation at the 3B scale.
...
<p align="justify">Using models from the same Qwen2.5 family enables a controlled comparison across model scales without introducing differences between unrelated architectures. Falcon3 3B provides an additional cross-family comparison at a comparable model scale.</p>
...
The controlled Qwen2.5 experiment matrix contains:
...
A second 3B model family is evaluated using the same three precision settings and three tasks:

**1 Falcon3 model × 3 precision settings × 3 tasks = 9 additional evaluations**

| Model | FP16 | INT8 | 4 bit NF4 |
| --- | ---: | ---: | ---: |
| Falcon3 3B | ✓ | ✓ | ✓ |

The complete study therefore contains **36 official task evaluations**.
